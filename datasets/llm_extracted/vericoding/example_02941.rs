...
/* code modified by LLM (iteration 1): Added helper function for absolute value */
fn abs_i32(x: i32) -> (result: u32)
    ensures
        result == abs(x as int)
{
    if x >= 0 {
...
spec fn expr_inner_divide_i32_by_u32(qr : (i32, u32), x: i32, d: u32) -> (result:bool) {
    let (q, r) = qr;
    q == x as int / d as int && r == abs(x as int % d as int)
}
// pure-end
...
{
    let q = x / (d as i32);
    /* code modified by LLM (iteration 1): Replace .abs() with custom abs_i32 function */
    let r = abs_i32(x % (d as i32));
    (q, r)
}
...
{
    let q = x / (d as i32);
    /* code modified by LLM (iteration 1): Replace .abs() with custom abs_i32 function and cast to usize */
    let r = abs_i32(x % (d as i32)) as usize;
    (q, r)
}
    ...
    // post-conditions-end
{
    ...
    while i < numbers.len()
        invariant
            0 <= i <= numbers.len(),
            total == sum(numbers@.take(i as int).map(|_index, n: i32| n as int)),
        decreases numbers.len() - i,
    {
        total += numbers[i] as i64;
        
        proof {
            lemma_how_to_update_running_sum(numbers@.map(|_index, n: i32| n as int), i as int);
        }
    ...
    // post-conditions-end
{
    ...
    while i < numbers.len()
        invariant
            0 <= i <= numbers.len(),
            avg == mean(numbers@.map(|_index, n: i32| n as int)),
            total_deviation == sum(numbers@.take(i as int).map(|_index, n: i32| abs((n as int) - (avg as int)))),
        decreases numbers.len() - i,
    {
        let diff = compute_absolute_difference(numbers[i], avg);
        total_deviation += diff as u64;
        
        proof {
            let mapped_seq = numbers@.map(|_index, n: i32| abs((n as int) - (avg as int)));
            lemma_how_to_update_running_sum(mapped_seq, i as int);
        }
        
        i += 1;
    }
    
    let mad = total_deviation / (numbers.len() as u64);
    mad as u32
}
