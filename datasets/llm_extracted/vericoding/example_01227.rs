    ...
    decreases seq.len()
{
    if seq.len() > 0 {
        ...
        /* code modified by LLM (iteration 1): replaced ext_equal with === */
    ...
    nums@.to_set() === res@.to_set()
{
    ...
    while i < nums.len()
        invariant
            ...
            /* code modified by LLM (iteration 1): fixed syntax error - replaced 'in' with '.contains()' */
            forall |j: int| 0 <= j < i ==> 
                (result@.to_set().contains(nums@[j]) <==> 
