    ...
    // post-conditions-end
{
    ...
    while idx <= main.len() - sub.len()
        invariant
            ...
            /* code modified by LLM (iteration 3): Fixed trigger to avoid arithmetic in trigger position */
            forall|k: int| 0 <= k < idx ==> {
                let subrange_end = k + sub@.len();
                !(#[trigger] main@.subrange(k, subrange_end) =~= sub@)
            },
        /* code modified by LLM (iteration 1): Added decreases clause to prove loop termination */
