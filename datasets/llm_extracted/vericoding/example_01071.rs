...
verus! {
    ...
    fn Test()
    {
        // Simple test function with no requirements or ensures
        let _ = Max(5, 10);
    }
    ...
    fn m1(x: i32, y: i32) -> (z: i32)
        ...
        ensures z >= 0 && z <= y && z != x,
    {
        // Since 0 < x < y, we know x >= 1 and y > x
        // We need z >= 0, z <= y, and z != x
        // A simple solution is to return 0 (since x > 0, so z != x is satisfied)
        0
    }
