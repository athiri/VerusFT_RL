...
spec fn first_zero_index(s: Seq<i32>) -> int {
    /* code modified by LLM (iteration 1): replaced while loop with recursive approach */
    first_zero_index_rec(s, 0)
}

spec fn first_zero_index_rec(s: Seq<i32>, i: int) -> int
    decreases s.len() - i
{
    if i >= s.len() {
        s.len() as int
