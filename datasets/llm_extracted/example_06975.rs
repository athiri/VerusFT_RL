    ...
    ensures longest_consecutive_postcond(nums, result as nat),
{
    ...
    // Build the HashSet
    /* code modified by LLM (iteration 2): Fixed loop invariant to use spec-level HashSet view */
    for i in 0..nums.len()
        invariant 
        ...
        invariant max_length <= nums.len(),
    {
        ...
        if !num_set.contains(&(num - 1)) {
            ...
            /* code modified by LLM (iteration 2): Fixed decreases clause syntax by removing 'when' */
            while current_num < i32::MAX && num_set.contains(&(current_num + 1))
                invariant 
                    current_length >= 1,
