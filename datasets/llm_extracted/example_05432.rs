...
{
    /* code modified by LLM (iteration 1): stored both values before mutable operations to avoid borrowing conflict */
    let temp = a[i];
    let temp_j = a[j];
    a.set(i, temp_j);
    a.set(j, temp);
        ...
        // Full postcondition would require complex invariants to prove
{
    let mut result = a;
    if result.len() > 1 {
        /* code modified by LLM (iteration 1): stored result.len() in a local variable to avoid borrowing conflict */
        let result_len = result.len();
        bubble_outer(&mut result, result_len - 2);
    }
    result
}
