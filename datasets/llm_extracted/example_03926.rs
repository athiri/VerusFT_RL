...
verus! {
            ...
            a.len() == old(a).len(),
    {
            ...
            decreases n - i
        {
            ...
            /* code modified by LLM (iteration 2): added explicit trigger annotation and refined invariants */
            while j < n - 1 - i
                invariant
                    i < n,
                    j <= n - 1 - i,
                    ...
                    forall|k: int| #[trigger] a[k] <= a[j + 1] && 0 <= k <= j,
                decreases n - 1 - i - j
            {
                if a[j] > a[j + 1] {
                    /* code modified by LLM (iteration 2): fixed borrowing issue by storing values in temporary variables before swap */
                    let temp_j = a[j];
