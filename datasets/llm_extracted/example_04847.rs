                ...
                ==> forall|j: int| #![auto] 0 <= j < c[i].len() ==> c[i][j] == a[i][j] + b[i][j],
{
    ...
    /* code modified by LLM (iteration 2): Added decreases clause for outer while loop */
    while i < a.len()
        invariant
            c.len() == i,
        ...
        decreases a.len() - i
    {
        ...
        /* code modified by LLM (iteration 3): Fixed decreases clause type casting to int */
        while j < a[i].len()
            invariant
                row.len() == j,
            ...
            decreases a[i].len() as int - j as int
        {
