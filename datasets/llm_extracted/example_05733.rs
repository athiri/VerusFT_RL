    ...
    // post-conditions-end
{
        ...
        decreases string.len() - i,
    {
        if i + substring.len() <= string.len() {
            /* code modified by LLM (iteration 1): create substring slice using Vec operations instead of range syntax */
            let mut string_slice = Vec::new();
            let mut j = i;
            while j < i + substring.len()
                invariant
                    i <= j <= i + substring.len(),
                    i + substring.len() <= string.len(),
                    string_slice.len() == j - i,
                    forall|k: int| 0 <= k < j - i ==> string_slice@[k] == string@[i + k],
                decreases i + substring.len() - j,
            {
                string_slice.push(string[j]);
                j += 1;
            }
            
            let prefix_check = is_prefix(substring.clone(), string_slice);
            if prefix_check {
                if count == u32::MAX {
                    return None;
                }
