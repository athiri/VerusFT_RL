        ...
        result@.len() == nums1@.len(),
{
            ...
            result@.len() == i,
    {
        ...
        for j in 0..nums2.len()
            invariant
                found_idx.is_none() ==> forall|k: int| 0 <= k < j ==> nums2@[k] != val,
                found_idx.is_some() ==> {
                    let idx = found_idx.unwrap();
                    idx < nums2.len() && nums2@[idx as int] == val
                },
        {
            if nums2[j] == val {
                found_idx = Some(j);
        ...
        // Since we know all elements of nums1 are in nums2, found_idx must be Some
        let idx = found_idx.unwrap();
        
        // Find next greater element starting from idx + 1
        let mut next_greater = -1;
        for k in (idx + 1)..nums2.len()
            invariant
                next_greater == -1 ==> forall|m: int| (idx as int) < m < k ==> nums2@[m] <= val,
                next_greater != -1 ==> {
                    exists|m: int| (idx as int) < m < nums2@.len() && nums2@[m] == next_greater && nums2@[m] > val
                },
        {
            if nums2[k] > val {
