            ...
            forall|x: int, y: int| 0 <= x < y < i ==> result@.index(x) <= result@.index(y),
    {
        ...
        while j < result.len()
            invariant
                ...
                s@.to_multiset() == result@.to_multiset(),
                forall|x: int, y: int| 0 <= x < y < i ==> result@.index(x) <= result@.index(y),
                forall|k: int| i < k < j ==> result@.index(i) <= result@.index(k),
        {
            if result[j] < result[i] {
                ...
                result.set(j, temp);
                proof {
                    swap_preserves_multiset(old(result)@, result@, i as int, j as int);
                }
            }
            j += 1;
        }
    ...
    // post-conditions-end
{
            ...
            forall|x: int| 0 <= x < i ==> result@.contains(s[x]),
    {
        if result.len() == 0 || result[result.len() - 1] != s[i] {
            result.push(s[i]);
            proof {
                lemma_seq_contains_after_push(result@, s[i as int]);
            }
        }
        i += 1;
    }
