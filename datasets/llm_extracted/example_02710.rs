...
fn pluck_smallest_even(nodes: &Vec<u32>) -> (result: Vec<u32>)
    ...
    ensures
        ...
        result@.len() == 2 ==> {
            &&& result@[0] % 2 == 0
            &&& 0 <= result@[1] < nodes@.len()
            &&& nodes@[result@[1] as int] == result@[0]
            &&& forall|i: int| 0 <= i < nodes@.len() ==> nodes@[i] % 2 == 0 ==> result@[0] <= nodes@[i]
            &&& forall|i: int| 0 <= i < result@[1] ==> nodes@[i] % 2 != 0 || nodes@[i] > result@[0]
        },
    // post-conditions-end
{
    // impl-start
