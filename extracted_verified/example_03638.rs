    ...
    ensures trap_rain_water_postcond(height@, result, trap_rain_water_precond(height@))
{
    ...
    while i < n
        ...
        decreases n - i
    {
        /* code modified by LLM (iteration 1): Fixed borrowing issue by storing previous value before set operation */
        let prev_val = left_max[i - 1];
        if height[i] > prev_val {
            left_max.set(i, height[i]);
        } else {
            left_max.set(i, prev_val);
        }
        i += 1;
    }
    ...
    while i > 0
        ...
        decreases i
    {
        /* code modified by LLM (iteration 1): Fixed borrowing issue by storing current value before set operation */
        let curr_val = right_max[i];
        if height[i - 1] > curr_val {
            right_max.set(i - 1, height[i - 1]);
        } else {
            right_max.set(i - 1, curr_val);
        }
        i -= 1;
    }
