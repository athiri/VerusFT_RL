    ...
    decreases ys@.len(),
{
    ...
    /* code modified by LLM (iteration 1): Fixed old() usage by capturing insert position and correcting invariants */
    while i < ys.len()
        invariant
            0 <= insert_pos <= i <= ys.len(),
            result@.len() == (i - insert_pos) + insert_pos + 1,
            forall|j: int| 0 <= j < insert_pos ==> result@[j] == ys@[j],
            result@[insert_pos as int] == x,
            forall|j: int| insert_pos <= j < i ==> result@[j - insert_pos + insert_pos + 1] == ys@[j],
        decreases ys@.len() - i,
    {
        result.push(ys[i]);
        i = i + 1;
    ...
    decreases arr@.len(),
{
    ...
    /* code modified by LLM (iteration 1): Fixed loop invariant bounds */
    for i in 1..arr.len()
        invariant
            tail@.len() == (i - 1) as int,
            forall|j: int| 0 <= j < (i - 1) as int ==> tail@[j] == arr@[j + 1],
    {
        tail.push(arr[i]);
    }
