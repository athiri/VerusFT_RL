...
verus! {
        ...
        requires 0 < sub.len() <= str.len(), // This method requires that sub is less than or equal in length to str
    {
                ...
                0 <= start_pos <= str.len() - sub.len() + 1,
        {
            ...
            while i < sub.len()
                invariant 
                    0 <= i <= sub.len(),
                    matches ==> forall|j: int| 0 <= j < i ==> sub[j as usize] == str[(start_pos + j as usize) as usize],
            {
    ...
    fn main() {
        ...
        println!("is_prefix: {}", is_pre);
        println!("is_substring: {}", has_sub);
        println!("have_common_k_substring: {}", has_common);
        println!("max_common_substring_length: {}", max_len);
    }
}