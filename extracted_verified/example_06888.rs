        ...
        max_subarray_sum_divisible_by_k_postcond(arr@, k, result),
{
    ...
    // Check all possible subarrays with length divisible by k
    let mut start = 0;
    while start < n {
        ...
        while start + len <= n {
            ...
            // Check if this sum is better than our current max
            if current_sum > 0 {
                if !found_positive || current_sum > max_sum as i64 {
                    // Ensure we don't overflow i32
                    if current_sum <= i32::MAX as i64 {
