    ...
    ensures all_characters_same_postcond(chars, result)
{
    ...
    proof {
        // At this point, we've checked all characters and they're all equal to first_char
        // Need to prove pairwise_equal holds
        assert(forall|j: int, k: int| 0 <= j < chars@.len() && 0 <= k < chars@.len() ==> chars@[j] == chars@[k]) by {
            // All characters equal the first character, so transitively all equal each other
