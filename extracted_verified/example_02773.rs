    ...
    // post-conditions-end
{
    ...
    while i < ret.len()
        invariant
            ...
            forall|x: int, y: int| 0 <= x < i && i <= y < ret@.len() ==> ret@.index(x) <= ret@.index(y),
            ret@.len() == s@.len(),
            s@.to_multiset() == ret@.to_multiset(),
    {
        let mut min_idx = i;
        ...
        if min_idx != i {
            /* code modified by LLM (iteration 1): Use tracked variable for old sequence and proper proof structure */
            let ghost old_ret = ret@;
            let temp = ret[i];
            ret.set(i, ret[min_idx]);
            ret.set(min_idx, temp);
            
            proof {
                swap_preserves_multiset(old_ret, ret@, i as int, min_idx as int);
            }
        }
        i += 1;
    }
    ...
    // post-conditions-end
{
    ...
    while i < s.len()
        invariant
            ...
            forall|x: int| #![auto] 0 <= x < result@.len() ==> s@.contains(result@.index(x)),
            forall|x: int| 0 <= x < i ==> result@.contains(s@.index(x)),
    {
