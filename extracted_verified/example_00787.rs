            ...
            a@.to_multiset() == old(a)@.to_multiset(),
    {
        ...
        while i < n
            invariant
                ...
                a@.to_multiset() == old(a)@.to_multiset(),
                a@.len() == old(a)@.len(),
                /* code modified by LLM (iteration 2): added explicit trigger annotations for nested quantifier */
                forall|k: int| #[trigger] (i <= k < n) ==> 
            ...
            decreases n - i,
        {
            ...
            while j < n - 1 - i
                invariant
                    ...
                    forall|k: int| #[trigger] (j < k < n - i) ==> a@[k - 1] <= a@[k],
                decreases n - 1 - i - j,
            {
                if a[j] > a[j + 1] {
                    /* code modified by LLM (iteration 1): fixed borrowing conflict by storing values before swap */
                    let temp = a[j];
                    let temp2 = a[j + 1];
                    a.set(j, temp2);
                    a.set(j + 1, temp);
