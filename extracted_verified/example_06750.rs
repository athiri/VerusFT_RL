    ...
    requires prev_nums.len() == lengths.len()
{
    ...
    while i < prev_nums.len()
        invariant 
            ...
            max_length <= 1000000
    {
        if prev_nums[i] < curr_num && lengths[i] > max_length {
    ...
    )
{
            ...
            forall|j: int| 0 <= j < i ==> lengths[j as int] <= 1000000
    {
        let prev_length = if i == 0 {
            ...
            let prev_slice = numbers.slice(0, i);
            let length_slice = lengths.slice(0, i);
            find_length_ending_at_curr(&prev_slice, &length_slice, numbers[i])
        };
