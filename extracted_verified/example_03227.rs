...
verus! {
            ...
            decreases f - i
        {
            ...
            while j < f - 1
                invariant
                    c <= j <= f - 1,
                    a.len() == old(a).len(),
                    /* code modified by LLM (iteration 1): added bounds invariant for array access */
                    f > 0,
                    f <= a.len(),
                decreases f - 1 - j
            {
                /* code modified by LLM (iteration 1): added bounds check before array access */
            ...
            a.len() == old(a).len(),
    {
        ...
        /* code modified by LLM (iteration 1): changed decreases clause to avoid multiset issue */
        let mut iterations = f - c;
        while swapped && iterations > 0
            invariant
                a.len() == old(a).len(),
                f <= a.len(),
