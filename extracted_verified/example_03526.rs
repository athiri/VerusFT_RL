    ...
    ensures has_opposite_sign_postcond(a, b, has_opposite_sign(a, b))
{
    ...
    // Case analysis on the signs of a and b
    if (a < 0 && b > 0) || (a > 0 && b < 0) {
        // If a and b have opposite signs, then a * b < 0
        // This follows from basic arithmetic properties
        assert(result);
    } else {
        // If a and b don't have opposite signs, then a * b >= 0
        // This covers cases where: both are positive, both are negative, or at least one is zero
        assert(!result);
    }
}
