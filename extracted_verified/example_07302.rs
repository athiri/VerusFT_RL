...
fn main() {
    /* code modified by LLM (iteration 1): Fixed integer literals and conversion for printing */
    let intervals: Vec<(int, int)> = vec![(1, 3), (2, 6), (8, 10), (15, 18)];
    let merged = merge_intervals(intervals);
    
    // Convert to regular integers for printing using truncate method
    let printable_merged: Vec<(i32, i32)> = merged.iter().map(|&(start, end)| (start.truncate(), end.truncate())).collect();
