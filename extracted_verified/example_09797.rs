        ...
        kth_element_postcond(arr@, k as nat, result as int),
{
    ...
    proof {
        ...
        // Show that witness_idx is in bounds
        assert(0 <= witness_idx < arr@.len()) by {
        ...
        // Show that arr@[witness_idx] == result and result == arr@[k as int - 1]
        assert(arr@[witness_idx] == result);
        assert(result as int == arr@[k as int - 1] as int);
    }
