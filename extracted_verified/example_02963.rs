    ...
    // post-conditions-end
{
    ...
    {
        /* code modified by LLM (iteration 1): store split_off result in variable to avoid complex argument error */
        if i + substring.len() <= string.len() {
            let mut string_suffix = string.clone();
            string_suffix.drain(0..i);
            if is_prefix(substring.clone(), string_suffix) {
                if count == u32::MAX {
                    return None;
                }
                count += 1;
