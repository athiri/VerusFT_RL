...
fn main() {
    /* code modified by LLM (iteration 1): Fixed type annotations and conversion for printing */
    let intervals: Vec<(int, int)> = vec![(1int, 3int), (2int, 6int), (8int, 10int), (15int, 18int)];
    let merged = merge_intervals(intervals);
    
    // Convert to regular integers for printing
    let printable_merged: Vec<(i32, i32)> = merged.iter().map(|&(start, end)| (start as i32, end as i32)).collect();
