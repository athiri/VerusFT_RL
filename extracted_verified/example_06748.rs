        ...
        majority_element_postcond(nums@, result),
{
    ...
    while i < nums.len()
        invariant
            0 < i <= nums.len(),
            count >= 0,
            // The key invariant: if we remove all the pairs of different elements
            // we've processed so far, the majority element still has more than
            // half of the remaining elements
            exists|maj: i32| count_in_seq(nums@, maj) > nums@.len() / 2 && 
                (count > 0 ==> candidate == maj)
