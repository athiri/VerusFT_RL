    ...
    ensures move_zeroes_postcond(xs@, result@)
{
    ...
    for i in 0..xs.len()
        invariant 
            ...
            result@ == filter_nonzeros(xs@.take(i as int)),
    {
        if xs[i] != 0 {
            result.push(xs[i]);
        } else {
    ...
    for i in 0..zero_count
        invariant 
            result.len() == filter_nonzeros(xs@).len() + i,
            result@.take(filter_nonzeros(xs@).len() as int) == filter_nonzeros(xs@),
            all_zeros(result@.skip(filter_nonzeros(xs@).len() as int)),
    {
        result.push(0);
    }
