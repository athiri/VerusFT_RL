        ...
        solution_precond(nums@),
{
    ...
    while i < n {
        let mut j = i;
        while j < n {
            let distinct_count = count_distinct_elements(&nums, i, j);
            let squared = (distinct_count * distinct_count) as u32;
            total_sum += squared;
            j += 1;
        }
        i += 1;
...
fn main() {
    ...
    let test_nums = vec![1, 2, 1];
    let result = solution(test_nums);
    // Note: println! is not supported in Verus
