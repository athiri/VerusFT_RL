        ...
        digits@.len() == 0 || (digits@.len() <= 10 && forall|j: int| 0 <= j < digits@.len() ==> digits[j] <= 1),
{
    ...
    while i < digits.len()
        invariant
            0 <= i <= digits.len(),
            binary_to_decimal_precond(digits@.map(|i: int, x: u32| x as nat)),
            forall|j: int| 0 <= j < digits@.len() ==> digits[j] <= 1,
    {
...
fn main() {
    let binary_digits = vec![1, 0, 1, 1];
    let decimal_result = binary_to_decimal(binary_digits);
    println!("Binary 1011 in decimal is: {}", decimal_result);
}
