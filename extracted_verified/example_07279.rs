...
{
    if start == l.len() - 1 {
        // Reached the end - valid if we found a peak and are decreasing
    ...
    if increasing {
        if current < next {
            // Continue increasing
            aux(l, start + 1, true, false)
        } else if current > next {
            // Found peak, start decreasing
            if start == 0 {
                // Need at least one increasing step before peak
                false
            } else {
                aux(l, start + 1, false, true)
            }
        } else {
            // Equal elements - not valid
            false
        }
    } else {
        // Must be decreasing
