    ...
    decreases l1.len() + l2.len()
{
    if l1.len() == 0 && l2.len() == 0 {
        ...
        let rest = add_aux(Vec::new(), l2.subrange(1, l2.len() as int), new_carry);
        result.append(rest);
        result
    } else if l2.len() == 0 {
        ...
        let mut result = vec![digit % 10];
        let new_carry = digit / 10;
        let rest = add_aux(l1.subrange(1, l1.len() as int), Vec::new(), new_carry);
        result.append(rest);
        result
    } else {
        ...
        let mut result = vec![digit % 10];
        let new_carry = digit / 10;
        let rest = add_aux(l1.subrange(1, l1.len() as int), l2.subrange(1, l2.len() as int), new_carry);
        result.append(rest);
        result
    }
}
