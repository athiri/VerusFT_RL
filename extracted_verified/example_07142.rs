        ...
        match prev {
            ...
            Some(prev_val) => {
                if current > prev_val {
                    ...
                    longest_increasing_streak_aux(nums, idx + 1, Some(current), curr_len + 1, max_len)
                } else {
                    // Break in streak, update max_len and start new streak
                    let new_max = if curr_len > max_len { curr_len } else { max_len };
                    longest_increasing_streak_aux(nums, idx + 1, Some(current), 1, new_max)
                }
            }
        }
    }
...
fn main() {
    let nums = vec![1, 3, 2, 4, 5, 1, 6, 7, 8];
